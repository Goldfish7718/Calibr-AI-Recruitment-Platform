...
export default function TechnicalInterviewClient({
  ...
  const session = useInterviewSession({
    ...
    adapter: {
      startEvaluation: async (id: string) => {
        await technicalInterviewAdapter.startEvaluation(id);
      },
      analyze: technicalInterviewAdapter.analyze,
      generateQ2Questions: async (q1Question: string, q1Answer: string, topicId: string) => {
        return await actions.generateQ2Questions(q1Question, q1Answer, topicId);
      },
    },
  });
  ...
  const flow = useInterviewFlow({
    ...
    adapter: {
      ...
      analyze: async (question: string, idealAnswer: string, userAnswer: string, queues: any, currentQuestion: any, interviewId: string) => {
        return await technicalInterviewAdapter.analyze(question, idealAnswer, userAnswer, queues, currentQuestion, interviewId);
      },
    },
  ...
  // Pause interview when tab is hidden (prevent audio playing in background)
  useEffect(() => {
    if (session.currentScreen !== "interview") return;

    const handleVisibilityChange = () => {
      if (document.hidden) {
        console.log("[Interview] ⚠️ Tab hidden - STOPPING all audio and processes");
        
        // STOP (not pause) any playing audio
        const audioElements = document.querySelectorAll("audio");
        audioElements.forEach((audio) => {
          audio.pause();
          audio.currentTime = 0; // Reset to start
          console.log("[Interview] Stopped audio element");
        });

        // CANCEL (not pause) speech synthesis - pause doesn't work reliably
        if (window.speechSynthesis) {
          if (window.speechSynthesis.speaking || window.speechSynthesis.pending) {
            window.speechSynthesis.cancel(); // This actually stops it
            console.log("[Interview] ✓ Cancelled speech synthesis");
          }
        }
        ...
        if (session.speechRecognitionRef.current) {
          try {
            ...
            console.debug("[Interview] Error stopping STT:", error);
          }
        }
        ...
        console.log("[Interview] ✓ All processes stopped due to tab hidden");
        ...
        console.log("[Interview] ✓ Tab visible again");
        // Don't auto-resume - let user manually continue
      }
    };
